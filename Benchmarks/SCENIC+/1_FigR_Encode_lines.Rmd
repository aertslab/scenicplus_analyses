...
rm(countsATAC)
save(ATAC.SE, file="int/ATAC.SE.RData")
ATAC.SE
```
...
cisCor <- runGenePeakcorr(ATAC.se = ATAC.SE,
                          ...
                          nCores = 10, 
                          p.cut=NULL)
save(cisCor, file="int/1_cisCor.RData")
date()
```
#### Determine DORCs
...
write.table(numDorcs, file="int/2_numDorcs.txt", row.names=F, quote=F)
numDorcs
```
...
```{r}
## Save for later...
# save(dorcMat.smooth, file="int/dorcMat.smooth.RData")
# save(RNAmat.smooth, file="int/RNAmat.smooth.RData")
# save(umap.d, file="int/cellsUmap.RData")
```

```{r}
## Reload if needed
# load("int/dorcMat.smooth.RData")
# load("int/RNAmat.smooth.RData")
# load("int/cellsUmap.RData")
```
...
fig.d <- runFigRGRN(ATAC.se=ATAC.SE,
                    ...
                    genome="hg38",
                    nCores=1)
save(fig.d, file="int/2_GRN_fig.d.RData")
date()
```
